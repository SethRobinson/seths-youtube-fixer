...
- **Feedback actions it captures from YouTube:** video IDs, channel IDs, video titles,
  channel names, and the "Not interested" and "Don't recommend channel" action tokens that
  YouTube itself puts on recommendation cards. These power the "Less like this" and
...
- It **never** stores your Google or YouTube password, cookies, or session tokens.
...
1. **YouTube (`youtube.com`):** using **your own existing logged-in session**, to perform
   the same actions the site already offers you (submit "Not interested" and "Don't recommend
   channel" feedback, pause or resume watch history) and to display the real YouTube page in
...
YouTube, My Activity, and the Google API, the ability to open helper tabs, and the ability to
